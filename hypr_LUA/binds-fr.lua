...
-- Send to Workspaces (AZERTY layout)
...
-- Screenshots (Requires hyprshot, gradia & wl-screenrec)
...
hl.bind("ALT + Print",        hl.dsp.exec_cmd("wl-screenrec --max-fps 60 --audio -f ~/Vidéos/$(date +%Y-%m-%d_%H-%M-%S).mp4"),           { locked = true })
hl.bind("SUPER + ALT + Print", hl.dsp.exec_cmd("kill $(pgrep wl-screenrec)"),                                                             { locked = true })

-- Power menu (Requires wlogout)
hl.bind("CTRL + ALT + Delete", hl.dsp.exec_cmd("wlogout"))

-- Color picker (Requires hyprpicker)
hl.bind(mainMod .. " + ALT + P", hl.dsp.exec_cmd("sleep 0.5 && hyprpicker -a"))
